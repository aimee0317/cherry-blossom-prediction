...
The development of cherry blossom depends highly on geographical locations and local climate. In this project, we retrieved climate data from National Oceanic and Atmospheric Administration {rnoaa} and Carbon Dioxide emission data {owidco2andothergreenhousegasemissions}. Based on the bloom date provided by organizer, we decided to do following data wrangling.  

The first step of incorporating weather data to blossom data is to find a weather station close to each city. We matched them roughly by latitude and longitude. We retrieved the daily data including the max temperature, min temperature and precipitation of a day and aggregated the mean values of those three parameters by year. We also aggregated these parameters by winter. As most cherry blossom happens in spring time, we expected that the weather data in winter also have high correlation with the prediction of bloom date. 

Inspired by USA-NPN data, we also created Accumulated Growing Degree Day (agdd) as a variable to predict bloom date. Our agdd data is calculated by using the formula: $\sum(\frac{\text{T_max}+\text{T_min}}{2} - \text{T_base})$. We chose our $\text{T_base}=0$, which is the parameter chosen by USA-NPN data. We also used the temperature data of December from the previous year and January and February in this year to calculate the agdd.

We also wanted to include global warming variables into our prediction model as a warmer weather will cause the cherry to bloom earlier. We used the yearly Carbon Dioxide emission data from countries and matched them to each blossom data by year.
...
### Forecasting 
To apply our model and make prediction for the coming ten years, we need to forecast the weather data and co2 emission till 2031. As we have daily weather data from noaa, we tried fitting an ARIMA model and an Exponential Smoothing model by daily granularity and aggregated them to obtain yearly forecast. To test our forecast model, we manually split the Tmax data before 2015 as training set and 2015 and after as testing set. We achieved a better result on ARIMA model, so we applied ARIMA model to the weather data. The daily data we had by model forecasting were wrangled in the way that was mentioned in previous data processing part. 

As for co2 data, we had granularity by year so we directly applied ARIMA on all co2 data and casted forecast till year 2031.
